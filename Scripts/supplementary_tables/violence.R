...
.violence_measurement <- measurement %>%
  ...
  unite(col = violence_criteria, 2:7, sep = ", ", na.rm = T) %>%
...
.violence_measurement[.violence_measurement == "not specified_violence"] <- "not specified"


.violence_prevalence <- list()
for (i in 18:22) {
  .violence_prevalence$sum[[i-17]] <- sum(!is.na(measurement[i]))
  .violence_prevalence$perc[[i-17]] <-  percent(sum((!is.na(measurement[i]))/sum(!is.na(measurement$violence_period))))
...
.violence_ft <- .violence_ft %>%
  ...
  merge_at(j = 2, i = 6:10, part = "body") %>%
  merge_at(j = 2, i = 11:12, part = "body") %>%
  merge_at(j = 2, i = 13:14, part = "body") %>%
